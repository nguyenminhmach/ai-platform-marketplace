...
export async function POST(req: Request) {
  const { userId, modelImageDataUrl, garmentImageDataUrls } = await req.json();
  ...
  try {
    const result = await submitOutfitSwapJob(userId, modelImageDataUrl, garmentImageDataUrls, randomUUID());
