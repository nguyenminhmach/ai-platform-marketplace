...
Khi số cảnh (N) NHIỀU HƠN số hành động/khoảnh khắc riêng biệt thực sự có trong ý tưởng gốc (chiều ngược lại với quy tắc trên): TUYỆT ĐỐI không được mô tả lại TRỌN VẸN cùng 1 hành động ở nhiều cảnh khác nhau (vd không được để 2 cảnh riêng biệt đều mô tả nhân vật quay đủ 1 vòng 360°, đi hết cả quãng đường, hay đứng dậy trọn vẹn — nhìn sẽ giống hành động đó lặp lại nhiều lần, RẤT SAI). Thay vào đó, chọn ĐÚNG 1 hành động có chuyển động kéo dài tự nhiên nhất trong ý tưởng gốc (ưu tiên quay người, di chuyển, đứng lên/ngồi xuống — không chọn hành động tĩnh như mỉm cười, liếc nhìn vì không có gì để chia) rồi CHIA NHỎ hành động đó thành các GIAI ĐOẠN TIẾN TRIỂN liên tiếp trải đều qua các cảnh dư ra — mỗi cảnh chỉ thể hiện ĐÚNG 1 PHẦN của hành động đó (ví dụ hành động "quay một vòng 360°" chia làm 2 cảnh: cảnh đầu mô tả xoay từ vị trí ban đầu tới khoảng nửa vòng, "end_pose" ghi rõ đã xoay tới góc nào; cảnh sau tiếp nối ĐÚNG từ góc độ đó trong "description", xoay nốt phần còn lại cho tới khi hoàn tất trọn vẹn 360°). Đây KHÔNG tính là bịa thêm hành động mới (không vi phạm quy tắc "không bịa thêm" ở trên) — chỉ là chia nhỏ 1 hành động đã có sẵn theo đúng tiến trình thời gian thực của chính hành động đó.
...
Khi số cảnh (N) NHIỀU HƠN số hành động/khoảnh khắc riêng biệt thực sự có trong ý tưởng gốc (chiều ngược lại với quy tắc trên): TUYỆT ĐỐI không được mô tả lại TRỌN VẸN cùng 1 hành động ở nhiều cảnh khác nhau (vd không được để 2 cảnh riêng biệt đều mô tả 1 nhân vật quay đủ 1 vòng 360°, đi hết cả quãng đường, hay đứng dậy trọn vẹn — nhìn sẽ giống hành động đó lặp lại nhiều lần, RẤT SAI). Thay vào đó, chọn ĐÚNG 1 hành động có chuyển động kéo dài tự nhiên nhất trong ý tưởng gốc (ưu tiên quay người, di chuyển, đứng lên/ngồi xuống) rồi CHIA NHỎ hành động đó thành các GIAI ĐOẠN TIẾN TRIỂN liên tiếp trải đều qua các cảnh dư ra — mỗi cảnh chỉ thể hiện ĐÚNG 1 PHẦN của hành động đó (ví dụ "quay một vòng 360°" chia làm 2 cảnh: cảnh đầu xoay tới khoảng nửa vòng, "end_pose" ghi rõ đã xoay tới góc nào; cảnh sau tiếp nối ĐÚNG từ góc đó, xoay nốt phần còn lại). Đây KHÔNG tính là bịa thêm hành động mới — chỉ là chia nhỏ 1 hành động đã có sẵn theo đúng tiến trình thời gian thực.
...
const SCENE_PROMPT_FROM_IMAGE_SYSTEM =
  `You are a screenwriter writing a short motion prompt (1-2 sentences, English) for the given image, to be used as an image-to-video generation prompt. Base it on: what's visible in the image, and the "hint for this specific scene" if given — those are the ONLY authoritative sources for what happens in THIS moment. The "overall story" is background/tone context only — it tells you the general setting, NOT what to animate here.
CRITICAL: do NOT pull in, repeat, or partially reuse an action from the overall story that is not part of this scene's specific hint — the story was already split into scenes elsewhere, and large actions (a turn, a walk, standing up...) may already be assigned to a DIFFERENT scene (sometimes split across several scenes). Re-adding that action here, even "just once more" or "just the start of it", makes the character appear to repeat the same motion multiple times across the final video — a real, visible defect. If the hint for this scene doesn't mention a turn/walk/etc., do not invent one just because the overall story mentions it somewhere.
